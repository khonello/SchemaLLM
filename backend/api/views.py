...
class HomeSchemaAPIView(APIView):
    ...
    def post(self, request):
        ...
        conversation = request.data.get('conversation', None)
...
class DetailSchemaAPIView(APIView):

    def get(self, request):

        schema_queryset = DatabaseSchemaModel.objects.all()
        serializer = DatabaseSchemaSerializer(schema_queryset, many= True)
        return Response(serializer.data, status= 200)

class ProjectSchemaAPIView(APIView):

    def get(self, request, project_id):

        project_queryset = DatabaseSchemaModel.objects.filter(title= project_id)
        serializer = DatabaseSchemaSerializer(project_queryset.first(), many= False)
        return Response(serializer.data, status= 200)

    def post(self, request, project_id):
        
        conversation = request.data.get("conversation", None)
        existing_schema = DatabaseSchemaModel.objects.filter(title= project_id).first()
        ...
        return Response(
            {
                'conversations': existing_schema.conversations,
                'title': existing_schema.title,
                'sql': existing_schema.sql,
                'json': existing_schema.json
            }, status= 201
        )
